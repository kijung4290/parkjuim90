...
const getBlogThumbnailUrl = (imageUrl) => {
  if (!imageUrl) return '';

  try {
    const url = new URL(imageUrl);
    // RSS의 s3 이미지는 약 365px이라 넓은 카드에서 흐려집니다.
    if (url.hostname === 'blogthumb.pstatic.net') url.searchParams.set('type', 'w2');
...
const parseItem = (item, index) => {
  ...
  const image = getBlogThumbnailUrl(imageFromHtml(descriptionHtml));
